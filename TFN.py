...
from Models.BackboneCNN import CNN, ComplexConv1d
...
class Base_FUNC_CNN(CNN):
    ...
    def __init__(self, in_channels=1, out_channels=10, kernel_size=15, clamp_flag=True, mid_channel=16):
        ...
        new_layer = ComplexConv1d(**args)
        self.layer1.replace_layer(0, new_layer)
    ...
    def forward(self, x):
        x_r , x_i = self.funconv(x)
        return super().forward(x_r , x_i)
...
class TFN_FT(Base_FUNC_CNN):
    """
    TFN with TFconv-FT as the first preprocessing layer
    FuncConv1d = TFconv_FT
...
class TFN_Morlet(Base_FUNC_CNN):
    ...
    FuncConv1d = TFconv_Morlet
    def __init__(self, mid_channel=16, **kwargs):
        self.funckernel_size = mid_channel * 5 - 1
        super().__init__(mid_channel=mid_channel, **kwargs)
...
if __name__ == '__main__':
    ...
    for Model in [Base_FUNC_CNN,TFN_FT,TFN_STTF,TFN_Morlet,TFN_Chirplet,Random_CNN]:
        print('\n\n'+"-"*50+'\n'+Model.__name__+'\n'+"-"*50)
        models = Model()
        models.to(device)
