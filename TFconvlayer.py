...
class BaseConv1d(nn.Module):
    ...
    def forward(self, input):
        ...
        output1 = result[self.phases[0]]
        output2 = result[self.phases[1]]
...
class TFconv_FT(BaseFuncConv1d):  # kernel_size = out_channels * 2 - 1
    ...
    def weightforward(self, lens, params, phase):
        ...
        if self.phases.index(phase) == 0:
            result = torch.cos(2 * math.pi * params[0] * T)
        else:
            result = torch.sin(2 * math.pi * params[0] * T)
        return result
...
class TFconv_STTF(BaseFuncConv1d):  # kernel_size = out_channels * 2 - 1
    ...
    def weightforward(self, lens, params, phase):
        if isinstance(lens, torch.Tensor):
            lens = int(lens.item())
        T = torch.arange(-(lens // 2), lens - (lens // 2)).to(params.device)
        sigma = torch.tensor(0.5).to(params.device)
        if self.phases.index(phase) == 0:
...
if __name__ == '__main__':
    ...
    for item in [TFconv_FT,TFconv_STTF, TFconv_Chirplet, TFconv_Morlet]:
        print(item.__name__)
