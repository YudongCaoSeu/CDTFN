...
model = getattr(Models, 'TFN_Morlet') \
            (in_channels=1, out_channels=1, kernel_size=8,
             clamp_flag=True, mid_channel=30).to(device)
print(model)

optimizer = torch.optim.SGD(model.parameters(), lr=0.005, momentum=0.9)
try:
    model=torch.load(r'C:\Users\caoyu\Desktop\可解释性\TFN-main\weight\model4')
except:
# train steps
    train_loss = []
    ...
    torch.save(model, r'C:\Users\caoyu\Desktop\可解释性\TFN-main\weight\model4')
    plt.plot(train_loss)


path=r'.\test2-6.mat'
data=loadmat(path)               
data = data['horiz_signals'][:] #为打开mat的实际文件名
test_data = data[:,:-1].astype(np.float32)
test_label = data[:,-1].astype(np.float32)
test_data = torch.Tensor(test_data).to(device)
test_data=test_data.unsqueeze(1)
feature_r,feature_i,y=model(test_data)
feature_r = feature_r.cpu().detach().numpy()
feature_i = feature_i.cpu().detach().numpy()
np.save('feature_r_2-6_Obs4.npy',feature_r)
np.save('feature_i_2-6_Obs4.npy',feature_i)
